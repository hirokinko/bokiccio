...
- 参照日時点の資産・負債・純資産と、独立選択した月の費用合計・内訳を分けた日常用overview
...
会計年度・月次期間、明示的なreporting classification、commodity別試算表に加え、参照日時点の現在残高・選択月費用overview、年度別の期首B/S、月次P/L、月末全勘定残高推移まで提供済みである。複数commodityは引き続き換算ruleなしに合算せず、表示commodityごとに分けるか、明示的な換算結果だけを使用する。
