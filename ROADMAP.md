...
- 資産・負債・純資産・収益・費用へのreporting classification
- 開始月、期首・期末を持つ会計年度と、年度内の月次期間
- 期首残高、繰越、表示commodityの設定
- 月次・年度を選択でき、勘定科目階層を保った試算表
- 月末・期末または任意基準日時点の貸借対照表（B/S）
- 単月・年度累計・通期を切り替えられる損益計算書（P/L）
- 月末時点の資産・負債・純資産と勘定科目別残高の推移
- 前月・前年同月・前年度との比較
- 集計値から承認済み仕訳へのdrill-down
- 比較期間と小計・合計の機械可読export
- 現金・現金同等物とcash-flow categoryを定義した後のキャッシュフロー計算書（C/F）

最初のsliceは会計年度・月次期間、試算表、B/S、P/L、資産推移を対象にする。現在のaccount文字列だけから会計区分を暗黙推論せず、明示的なreporting classificationを導入する。複数commodityは換算ruleなしに合算せず、表示commodityごとに分けるか、明示的な換算結果だけを使用する。
...
年度・月次はreporting calendarとして定義し、entryの日付を対応する期間へ決定的に割り当てる。最初のsliceでは期間選択と再集計を扱い、月次締め・年度締めによる過去仕訳の編集制限や決算振替の自動生成は別途仕様を決める。
