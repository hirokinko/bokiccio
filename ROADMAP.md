...
会計年度・月次期間、明示的なreporting classification、commodity別試算表は提供済みである。次のsliceでは同じ分類・calendar・正確な集計基盤を使い、B/S、P/L、資産推移を追加する。複数commodityは引き続き換算ruleなしに合算せず、表示commodityごとに分けるか、明示的な換算結果だけを使用する。
